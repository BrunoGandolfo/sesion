# [HISTÓRICO] Cifrado de notas clínicas — fase 1 (columnas legacy en paralelo)

> Describe la fase 1 del cifrado en reposo, cuando las columnas en texto plano
> convivían con las `*_encrypted` y existía un script de backfill. La fase 4
> elimina las columnas legacy. La versión vigente está en `docs/encryption.md`.
> Este texto se conserva por el procedimiento de rollback y por la explicación
> del formato del blob, que no cambió.
