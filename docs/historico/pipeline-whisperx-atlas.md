# [HISTÓRICO] Pipeline de audio con WhisperX y Ollama en Atlas

> Este documento describe una arquitectura que **ya no existe**. Se conserva
> como registro de decisiones. La versión vigente está en `docs/pipeline.md`.
>
> Qué cambió desde entonces (verificado en `processor/config.py`,
> `processor/asr_assemblyai.py`, `processor/clinical_analyzer.py`):
> - El ASR es AssemblyAI (`universal-3-5-pro`, fallback `universal-2`), no WhisperX.
> - El LLM es Anthropic (`claude-sonnet-5`), no Ollama.
> - El worker corre en Railway, no en el box GPU "Atlas". No hay Tailscale ni GPUs.
> - El audio se descifra en memoria; no hay `AUDIO_TEMP_DIR` ni `.wav` en disco.
> - El navegador sube el audio directo a R2 con URL prefirmada (no pasa por Vercel).
> - El audio **no** se borra al terminar el procesamiento: vive hasta que la
>   profesional aprueba la nota o elimina la sesión.
> - Hot words: el modelo `HotWord` sigue en el schema pero el worker no lo consulta.
