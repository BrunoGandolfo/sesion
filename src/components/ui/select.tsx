...
export function Select({ label, id, className = "", children, ...props }: SelectHTMLAttributes<HTMLSelectElement> & { label: string }) {
  ...
  return <div className="flex flex-col gap-2">
    ...
    <select id={campoId} className={`min-h-11 w-full rounded-sm border border-[color:var(--border-subtle)] bg-cream-50 px-[14px] py-[10px] text-[15px] text-ink-900 transition-colors duration-[var(--duration-fast)] focus:border-sage-500 focus:bg-white focus:ring-[3px] focus:ring-sage-500/20 ${className}`} {...props}>{children}</select>
  </div>;
}
