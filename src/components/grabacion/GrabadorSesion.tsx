...
//     a que el navegador mate el proceso; cada chunk se cifra con la clave
//     de la sesión antes de escribirse (grabacion-storage.ts): en el
//     teléfono no queda audio en claro;
...
export interface GrabacionPendienteUI {
  /** Cifrados en el teléfono: se abren con la clave de la sesión al enviar. */
  chunks: ChunkCifrado[];
...
export interface Grabador {
  ...
  /** `claveCifrado`: la clave AES de la sesión (base64), la que entrega
   *  POST [id]/clave. Cifra cada chunk antes de guardarlo y el archivo
   *  entero antes de subirlo. */
  iniciar: (clave: string, claveCifrado: string) => Promise<void>;
  pausar: () => void;
  reanudar: () => void;
  terminar: () => void;
  descartar: () => void;
  reanudarTrasInterrupcion: () => Promise<void>;
  /** La clave se pide de nuevo al servidor: nunca quedó en el teléfono. */
  enviarPendiente: (claveCifrado: string) => void;
...
export function useGrabador({
  ...
  // La clave AES de la sesión. Vive sólo en memoria de esta pestaña: si la
  // página se recarga, la pantalla la vuelve a pedir al servidor.
  const claveCifradoRef = React.useRef<string | null>(null);
  ...
  async function procesarGrabacion(mimeType: string) {
    ...
    const claveCifrado = claveCifradoRef.current;
  ...
  function conectarRecorder(stream: MediaStream) {
    ...
    recorder.ondataavailable = (event: BlobEvent) => {
      if (event.data.size > 0) {
        ...
        if (clave && claveCifrado) {
          // Fire-and-forget: el backup en disco nunca bloquea la grabación.
          // Se cifra adentro, antes de escribir.
          void guardarChunk(clave, chunkIndiceRef.current, event.data, claveCifrado);
        }
  ...
  function restaurarTrasPageHide() {
    ...
    void recuperarGrabacionPendiente().then(async (recuperada) => {
      ...
      const chunks = await descifrarChunks(recuperada.chunks, claveCifrado).catch(
        () => null,
      );

      if (!chunks || estadoRef.current !== "inactivo") {
        return;
      }

      chunksRef.current = chunks;
      chunkIndiceRef.current = chunks.length;
  ...
  async function iniciar(clave: string, claveCifrado: string) {
    claveRef.current = clave;
    claveCifradoRef.current = claveCifrado;
  ...
  function enviarPendiente(claveCifrado: string) {
    ...
    void descifrarChunks(recuperada.chunks, claveCifrado).then(
      (chunks) => {
        chunksRef.current = chunks;
        chunkIndiceRef.current = chunks.length;
        pausasRef.current = recuperada.pausas;
      ...
      () => {
        // La clave no abre lo guardado (otra sesión, otra clave): no se
        // borra nada, se avisa. Lo pendiente se vuelve a ofrecer al entrar.
        setPendiente(recuperada);
        irAError("No se pudo abrir la grabación guardada en este teléfono.");
      },
    );
  }
