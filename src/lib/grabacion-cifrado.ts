...
//
// LA CLAVE LA DA EL SERVIDOR
//
// AES-GCM de 256 bits, una por sesión clínica: la genera el servidor al crear
// la sesión (casos-uso/audio.ts, prepararAudio) y el teléfono la pide por
// POST [id]/clave antes de grabar. Es la misma clave con la que se cifra
// cada trozo antes de guardarlo en el navegador (grabacion-storage.ts): así
// una grabación que quedó en el teléfono se puede abrir después de recargar,
// pidiendo la clave de nuevo, sin guardarla nunca en el dispositivo. El
// servidor la borra cuando la nota se aprueba: sin la clave, el blob que
// quede en R2 es ruido. Ese es el crypto-shredding del que depende la
// eliminación del audio.

import { cifrar, base64ABytes } from "@/lib/crypto";
...
export interface CifrarGrabacionParams {
  ...
  /** La clave AES-256 de la sesión, base64, tal como la entregó el servidor. */
  claveCifrado: string;
