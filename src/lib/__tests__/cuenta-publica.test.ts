...
it.each(["/registro", "/terminos", "/api/cuenta/registro", "/recuperar", "/restablecer", "/api/cuenta/recuperar", "/api/cuenta/restablecer"])("permite %s sin sesión y conserva CSP", async ruta => {
...
it.each(["/", "/config", "/api/cuenta/invitaciones", "/api/cuenta/password", "/api/cuenta/recuperar/otra"])("mantiene protegida %s", async ruta => {
  const request = Object.assign(new NextRequest(`https://sesionapp.app${ruta}`), { auth: null });
  const respuesta = await middleware(request, { params: Promise.resolve({}) });
