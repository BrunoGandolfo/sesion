...
const segmento = { indice: 0, inicioMs: 0, iv: "AAAAAAAAAAAAAAAA", bytes: 50, sha256: "a".repeat(64) };
test.each([{ key: "otra/sesion/0" }, { indice: -1 }, { iv: "corto" }, { bytes: 0 }, { bytes: 10_000_000 }, { sha256: "falso" }, { inicioMs: null }, { inicioMs: undefined }, { inicioMs: -1 }, { inicioMs: 60001 }, { inicioMs: 1 }, { inicioMs: Infinity }])("rechaza descriptor o ubicación inválida %j", cambio => {
  expect(segmentoAudioSchema.safeParse({ ...segmento, ...cambio }).success).toBe(false);
});
test("conserva la medida fraccionaria y no sustituye por el solape nominal", () => {
