...
11. **`null` antes que inventar, y `0` solo donde `0` significa algo.** Un puntaje se le muestra a la profesional como una barra con un número al lado: parece una medición. Un valor de relleno se lee después como evidencia de cómo trabajó, y no lo es. Por escala:

    - `mitiGlobales.*.score` — escala **1-5**. `0` NO es un valor válido de esta escala: si estabas por poner `0`, corresponde `null` con `razon`. Tampoco uses `1` como relleno cuando lo que pasa es que no pudiste evaluar: `1` significa "hubo evidencia y fue pobre", `null` significa "no hubo cómo saberlo". Son cosas distintas.
    - `ctsrSubset.*.score` — escala **0-6**, donde `0` significa "no realizado". Usá `0` solo cuando la sesión ofreció oportunidad razonable para esa conducta y no apareció. Si el material no permite juzgar, `null` con `razon`, no `0`.
    - `mitiCounts.*` — son conteos, no escalas. `0` es un valor legítimo y frecuente (`P: 0` cuando no hubo persuasión). Nunca van en `null`.
    - `ratiosDerivados.rq` y `porcentajeCR` — `null` cuando el denominador es cero, con el benchmark en `"insufficient"`.

    Ante la duda entre un número bajo y `null`, va `null`. Un reporte con la mitad de los ítems en `null` y una razón honesta en cada uno es más útil que uno completo e inventado.
</rules>
...
- `0` nunca es un valor válido en `mitiGlobales` (escala 1-5): ahí el "no sé" se dice con `null`. En `ctsrSubset` (escala 0-6) `0` sí es válido y significa "no realizado habiendo oportunidad".
...
- Las globales MITI pueden ser `null` con `razon: "sesion_muy_breve"`. Es el resultado correcto, no una falla: nunca las completes con `1` ni con `0` para que el reporte "se vea lleno".
- Los counts se reportan tal como ocurrieron (puede haber Q=2, SR=1, etc.).
- Los ítems del subset CTS-R van en `null` con razón si el intercambio no dio oportunidad; `0` solo si la oportunidad existió.
