...
import { visible, alcanzable, presentacion, fechaDePrueba } from './comprobaciones.mjs';
...
const pendientesOla2 = process.argv.includes('--pendientes-ola-2');
const erroresOla2 = [/^No pudimos completar la operación\. Intentá de nuevo\.$/];
const inicio = Date.now();
const salida = fileURLToPath(new URL('./resultados/', import.meta.url));
await mkdir(salida, { recursive: true });
const resultado = { origen, fecha: new Date().toISOString(), pasos: [], omitidos: [], esperados: [], restauracion: null };
...
async function agenda(prueba) {
  ...
  await paso('Agendar real: guardar turno pasado sin recordatorio', async () => {
    ...
    await escribir('POST', '/api/turnos', body => {
      ...
      assert.equal(json.data.serie, null, 'Se creó una serie inesperada');
    });
    await d.waitFor({state:'hidden'});
    const envios = await get('/api/sms/envios?turnoId=' + prueba.turnoId);
    assert.equal(envios.length, 0, 'Se programó un SMS para el turno pasado');
  ...
  await paso('Agenda: detalle del turno y reprogramación sin guardar', async () => {
    ...
    await alcanzable(boton('Reprogramar',d));
    await boton('Reprogramar',d).click();
    await visible(d.getByLabel('Fecha',{exact:true}));
    await alcanzable(boton('Guardar',d));
    await boton('Volver',d).click();
...
async function preparar() {
  ...
  for (let i = 0; i < Math.min(lista.length, 40) && !sesion; i += 4) {
    ...
    for (const { f, docs } of lote) {
      const aprobada = docs.sesiones.find(s => s.estado === 'aprobada' && s.feedback);
...
try {
  ...
  await context.route('**/*', async route => {
    ...
    if (u.origin === origen && u.pathname === '/api/cuenta/entrar' && metodo === 'POST') {
      const body = r.postDataJSON();
      if (body.email !== process.env.E2E_USUARIO || body.password !== process.env.E2E_PASSWORD) {
  ...
  page.on('response', r => {
    ...
    if (pendientesOla2 && r.status() === 404 && /^\/api\/pacientes\/[^/]+\/(brief|contexto-clinico)$/.test(ruta)) {
      resultado.esperados.push({pantalla,ancho,ruta,estado:404,motivo:'Backend retirado para la Ola 2; no se reconstruye en esta rama'});
    } else if (r.status() >= 400 && ruta.startsWith('/api/')) errores.push('HTTP ' + r.status() + ' ' + ruta);
  });
  await publicas();
  ...
  if (pendientesOla2) resultado.esperados.push({pantalla:'Grabación y subida',motivo:'No ejecutadas: upload-url/upload-confirmar retiradas para Ola 2'});
  await finalizar();
